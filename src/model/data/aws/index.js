...
async function deleteFragment(ownerId, id) {
  ...
  const deleteFromDynamoDB = new DeleteCommand(dynamoDbParams);
  ...
  const deleteFromS3 = new DeleteObjectCommand(s3Params);

  try {
    // Delete metadata from DynamoDB
    await ddbDocClient.send(deleteFromDynamoDB);

    // Delete data from S3
    await s3Client.send(deleteFromS3);

    logger.debug(
      { ownerId, id },
      "Fragment deleted successfully from DynamoDB and S3"
    );
  } catch (err) {
    logger.error(
      { err, ownerId, id },
